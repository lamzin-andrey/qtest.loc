...
(function () {
	...
	function init() {
		...
		UserTest.quests.push({q:"Вы набираете код внутри метода контроллера, в котором вам доступен класс Product, созданный ранее в этом тесте. Вы хотите записать последние изенения в базу данных. Введите соответствущую строку php кода.", a:"$em->flush();"});
		...
		UserTest.quests.push({q:"Вы набираете код внутри контроллера. Вы получили объект авторизованного пользователя в переменную $user с помощью строки кода $user = $this->get(\"security.context\")->getToken()->getUser(); Вы собираетесь получить объект шифровальшика в переменную $encoder. Введите соответствующую строку php кода ($encoder = ...;).", a:"$encoder = $this->get(\"security.encoder_factory\")->getEncoder($user);"});
		UserTest.quests.push({q:"Вы набираете код внутри контроллера. Вы получили объект шифровальшика в переменную $encoder с помощью строки кода $encoder = $this->get(\"security.encoder_factory\")->getEncoder($user); $user при этом имеет UserInterface. Вы собираетесь зашифровать пароль, хранящийся в переменной $password. Введите соответствующую строку php кода ($password = ...;).", a:"$password = $encoder->encodePassword($password, $user->getSalt());"});
		...
		UserTest.quests.push({q:"Как должна называеться секция, в которой должны располагаться ключи  class и property (в контексте четырех посследних вопросов)", a:"entity"});
